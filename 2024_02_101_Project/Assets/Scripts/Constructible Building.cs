...
public class ConstructibleBuilding : MonoBehaviour
{
    ...
    public int requriedTree = 5;                                     //건물 건설에 필요한 나무 숫자
    public float contructionTime = 2.0f;                             //건물 건설 시간
    ...
    public void StartConstruction(PlayerInventory inventory)
    {
        ...
        if (inventory.treeCount >= requriedTree)                           //건설에 필요한 나무 숫자를 확인한 후
        {
            inventory.Removeitem(ItemType.Tree, requriedTree);             //해당 나무 숫자 만큼 차감
        ...
        else
        {
            if (FloatingTextManager.Instance != null )
            {
                FloatingTextManager.Instance.Show($"나무가 부족합니다! ({inventory.treeCount} / {requriedTree})", transform.position +  Vector3.up);
            }
        }
    }
    private IEnumerator CostructionRoutine()
    {
        ...
        while (timer < contructionTime)
        {
            timer += Time.deltaTime;
            color.a = Mathf.Lerp(0.5f, 1f, timer / contructionTime);
            buildingMaterial.color = color;
            yield return null;
        }
        isConstructed = true;
