...
public class GridBuildingSystem : MonoBehaviour
{
    ...
    [SerializeField] private GameObject cellPrefab;             //셀의 프리팹
    ...
    void Update()
    {
        ...
        if(lookPosition != Vector3.zero)        //포지션이 0일때
        {
    ...
    {
        if (playerController.isFirstPerson) //플레이어의 1인칭 모드
        {
    ...
    private void OnDrawGizmos()      //Gizmo를 표시해주는 함수
    {
        ...
        for(int x = 0; x < width; x++)
        {
            for(int z = 0; z < height; z++)
            {
                ...
                Gizmos.DrawWireCube(cellCenter, new Vector3(cellSize, 0.11f, cellSize));      //각각의 칸을 그려준다.
            }
        }
    }
}
